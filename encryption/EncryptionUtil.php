...
namespace Songyichao\Kscnd\Encryption;
...
class EncryptionUtil
{
    ...
    {
        $encrypKeyAlg = self::getKeyEncryptionAlgm($encryptionMaterials);
        if ($encrypKeyAlg === "AES") {
            $secretKey = $encryptionMaterials;
            $encryptedSek = self::encode_AES_ECB($cek, $secretKey);
    ...
    public static function decodeCek($encryptionMaterials, $cekEncrypted)
    {
        ...
        if ($encrypKeyAlg === "AES") {
            $secretKey = $encryptionMaterials;
            $cek = self::decode_AES_ECB($cekEncrypted, $secretKey);
            if (empty($cek))
    ...
    public static function plainTextLength($args)
    {
        if (isset($args["Content"])) {
            if (is_array($args["Content"])) {
                ...
                $lengthInMeta = self::metaTextLength($args);
    ...
    public static function initMultipartUploadContext($initResult, $iv, $cek, $encryptedCek, $matdesc = "{}")
    {
        ...
        $file = self::openfile($encryptionDir . $initResult["UploadId"], "w");
        fwrite($file, $json);
        fclose($file);
    }
    ...
    {
        $encryptionDir = KS3_API_PATH . DIRECTORY_SEPARATOR . "cache" . DIRECTORY_SEPARATOR . "encryption" . DIRECTORY_SEPARATOR;
        $content = self::getMultipartUploadContext($UploadId);
        $content["nextIv"] = base64_encode($iv);
        $content["lastPart"] = $lastPart;
        $json = json_encode($content);
        $file = self::openfile($encryptionDir . $UploadId, "w");
        fwrite($file, $json);
        fclose($file);
    }
    ...
    {
        $encryptionDir = KS3_API_PATH . DIRECTORY_SEPARATOR . "cache" . DIRECTORY_SEPARATOR . "encryption" . DIRECTORY_SEPARATOR;
        if (!self::multipartUploadContextExists($UploadId))
    ...
    public static function createInstructionFile($bucket, $key, $cek, $iv, $matdesc = "{}")
    {
        ...
        $key = $key . self::$INSTRUCTION_SUFFIX;
