<?php
namespace Songyichao\Kscnd;

//使用时请在项目中引用该php文件
//设置默认时区
use Songyichao\Kscnd\Core\API;
use Songyichao\Kscnd\Core\Headers;
use Songyichao\Kscnd\Core\Ks3Request;
use Songyichao\Kscnd\Core\Logger;
use Songyichao\Kscnd\Core\MessageHolder;
...
class Ks3Client
{
    ...
    private function invoke($method, $args = [], $holder, $location = NULL)
    {
        $api = API::$API[$method];
        if (!$api) {
        ...
        if (empty($args[$position])) {
            ...
            $key = $args[$position];
            $preEncoding = mb_detect_encoding($key, ["ASCII", "UTF-8", "GB2312", "GBK", "BIG5"]);
        ...
        if ($signer === NULL || !($signer instanceof QueryAuthSigner)) {
            ...
            if ($data->status == 307) {
                ...
                if (substr($location, 0, 4) == "http") {
                    ...
                    return $this->invoke($method, [$args], $holder, $location);
                }
            }
            $holder->msg .= "response code->" . $httpRequest->get_response_code() . "\r\n";
            $holder->msg .= "response headers->" . serialize($httpRequest->get_response_header()) . "\r\n";
    ...
    //用于生产表单上传时的签名信息
    public function postObject($bucket, $postFormData = [], $unknowValueFormFiled = [], $filename = NULL, $expire = 18000)
    {
        $policy = [];
